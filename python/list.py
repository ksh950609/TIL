...
for _ in range(5):
    ...
    # 문자열 변수를 초기화 할 떄 는 '', "" 를 이용한다 
    # 문자열 안에 큰 따운표나 작은 따옴표가 포함되어야 하는 경우가 있습니다 
    # 전체 문자열을 큰따옴표로 구성하는 경우 내부적으로 작은따옴표를 포함할 수 있다 
    # 전체 문자열을 작은 따옴표로 구성 시  내부적으로 큰따옴표를 포함함
    # 혹은 백슬래시 | 를 사용하면 "" , ''를 원하는 만큼 포함 시킬 수 있다 
    
    
    
    # 문자열 변수에 덧셈을 이용하면 문자열이 더해져서 연결됨
    # 문자열 변수를 특정한 양의 정수와 곱하는 경우 문자열이 그 값 만큼 여러 번 더해짐
    # 문자열에 대해서도 마찬가지로 인덱싱과 슬라이싱을 이용
    # 더먼 문자열은 특정 인덱스의 값을 변경할 수 없음
    
    
    # 튜플 자료형은 리스트와 유사하지만 다음과 같은 문법적 차이가 있다 
    # 튜플은 한번 선언된 값을 변경할 수 없다
    # 리스트는 대과호 ([])을 이용하지만 튜플은 소괄호()를 사용
    ...
    # 튜플은 변경이 불가능해서 리스트와 다르게 키 값으로 사용 될 수 있다 
